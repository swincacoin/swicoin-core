...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered swi address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SWI.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal swi:URI (swi:XyZ...?message=example)
            {
...
static void NotifyzSWIReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSWIReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSWIReset.connect(boost::bind(NotifyzSWIReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSWIReset.disconnect(boost::bind(NotifyzSWIReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zswiTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
