...
#ifndef SWI_ZSWIWALLET_H
#define SWI_ZSWIWALLET_H
...
public:
    CzSWIWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSWI(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSWI(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SWI_ZSWIWALLET_H
