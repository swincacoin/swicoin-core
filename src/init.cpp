...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "swi.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "swid.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SWI and zSWI money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SWI/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SWI/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, swi, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SWI/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SWI specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-swistake=<n>", strprintf(_("Enable or disable staking functionality for SWI inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zswistake=<n>", strprintf(_("Enable or disable staking functionality for zSWI inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzswi=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSWI minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zswibackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSWI backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeswiamount=<n>     " + strprintf(_("Keep N SWI anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The SWI Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that SWI is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SWI Core is shutting down."));
...
#endif
    // Make sure only a single SWI process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SWI Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //SWI: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //SWI specific: zerocoin and spork DB's
                ...
                // SWI: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SWI Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SWI Core to complete") << "\n";
        ...
        zwalletMain = new CzSWIWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSWIWallet
        uiInterface.InitMessage(_("Syncing zSWI wallet..."));

        bool fEnableZSwiBackups = GetBoolArg("-backupzswi", true);
        pwalletMain->setZSwiAutoBackups(fEnableZSwiBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zswiTracker->Init();
...
//    }
//
//    nAnonymizeSwiAmount = GetArg("-anonymizeswiamount", 0);
//    if (nAnonymizeSwiAmount > 999999) nAnonymizeSwiAmount = 999999;
//    if (nAnonymizeSwiAmount < 2) nAnonymizeSwiAmount = 2;
    ...
    LogPrintf("Anonymize SWI Amount %d\n", nAnonymizeSwiAmount);
    ...
    /* Denominations
       ...
       For example:
       1SWI+1000 == (.1SWI+100)*10
       10SWI+10000 == (1SWI+1000)*10
    */
