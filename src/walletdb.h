...
public:
    ...
    bool WriteZSWISeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSWISeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSWISeed_deprecated(uint256& seed);
    bool EraseZSWISeed();
    bool EraseZSWISeed_deprecated();

    bool WriteZSWICount(const uint32_t& nCount);
    bool ReadZSWICount(uint32_t& nCount);
