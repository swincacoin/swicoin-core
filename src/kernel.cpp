...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSwiStake(spend));
    } else {
        ...
        CSwiStake* swiInput = new CSwiStake();
        swiInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(swiInput);
    }
