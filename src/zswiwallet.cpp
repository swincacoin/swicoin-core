...
CzSWIWallet::CzSWIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zswi seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSWISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSWISeed_deprecated()) {
                    LogPrintf("%s: Updated zSWI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zswi seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zswi wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSWIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSWICount(nCountLastUsed);
    else if (!walletdb.ReadZSWICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSWIWallet::Lock()
{
    seedMaster = 0;
}

void CzSWIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSWIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSWI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSWIWallet::LoadMintPoolFromDB()
{
...
void CzSWIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSWIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSWIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zswiTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSWIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSWI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zswiTracker which also adds to database
    pwalletMain->zswiTracker->Add(dMint, true);
...
void CzSWIWallet::SeedToZSWI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSWIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSWIWallet::GenerateDeterministicZSWI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSWIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSWIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
