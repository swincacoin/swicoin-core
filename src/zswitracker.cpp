...
CzSWITracker::CzSWITracker(std::string strWalletFile)
{
...
CzSWITracker::~CzSWITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSWITracker::Init()
{
...
bool CzSWITracker::Archive(CMintMeta& meta)
{
...
bool CzSWITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSWITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSWITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSWITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSWITracker::GetSerialHashes()
{
...
CAmount CzSWITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSWITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSWITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSWITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSWITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSWITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSWITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSWITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSWITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSWITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSWITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSWITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSWITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSWITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSWITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSWITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSWITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzswi from DB\n", __func__, listDeterministicDB.size());
    }
